...
func getBetByMatchIdAndOid(matchId string, orderId string) ([]Bet, error) {
	var footViews = make([]FootView, 0)
	if err := mysql.DB.Model(&FootView{}).Where("match_id=? ", matchId).Find(&footViews).Error; err != nil {
		log.Error("根据比赛id 查比赛详情失败")
		return nil, err
	...
	var betResult = make([]Bet, 0)
	if err := mysql.DB.Debug().Model(Bet{}).Where("id In ? and order_id = ?", ids, orderId).Find(&betResult).Error; err != nil {
		log.Error("查询bet 失败！")
		log.Error(err)
		return nil, err
	}
